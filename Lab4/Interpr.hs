...
parsePower = parseBinOpSequence parseJmpIf expectPowerOp

parseJmpIf :: Parser Expression
parseJmpIf = parseBinOpSequence parseTerm expectJmpIfOp
...
data RpnCommand = RpnConst Int
               | RpnUnOp  UnOp
               | RpnBinOp BinOp
               deriving (Show)

-- Some corrections were made to facilitate jump command

execRpn :: [RpnCommand] -> Int -> RpnCommand -> [Int] -> ([Int], [RpnCommand], Int)
execRpn ors p (RpnConst x) zs = (x:zs, drop (p+1) ors, p+1)
execRpn ors p (RpnUnOp op) (z:zs) = if op == UnOpJmp then let (newOpStack, newP) = doJmp ors p z in (zs, newOpStack, newP)
                                    else (((unOpSemantics ! op) z) : zs, drop (p+1) ors, p+1)
execRpn ors p (RpnBinOp op) (u:(v:zs)) = if op == BinOpJmpif then if u == 0 then let (newOpStack, newP) = doJmp ors p v in (zs, newOpStack, newP)
                                                                  else (zs, drop (p+1) ors, p+1)
                                         else (((binOpSemantics ! op) u v) : zs, drop (p+1) ors, p+1)
 
runRpn :: [RpnCommand] -> [Int]
runRpn xs = runRpnHelper xs 0 xs []

runRpnHelper:: [RpnCommand] -> Int -> [RpnCommand] -> [Int] -> [Int]
runRpnHelper [] p ors ys = ys
runRpnHelper (x:xs) p ors ys = let (stack, opStack, newP) = execRpn ors p x ys
                               in runRpnHelper opStack newP ors stack
...
data JmpOp = JmpNCommands
		  deriving (Show, Eq, Ord)
		 
data JmpOpIf = JmpNCommandsIf
            deriving (Show, Eq, Ord)
			
doJmp :: [RpnCommand] -> Int -> Int -> ([RpnCommand], Int)
doJmp opStack p shift = let currP = p + shift
                        in if currP < 0 then (opStack, 0) 
                           else let size = length opStack
						        in if currP >= size then ([], size)
							       else (drop currP opStack, currP)
			
stripPrefixX:: [Char]->[Char]->Maybe[Char]
stripPrefixX xs ys = let rest = stripPrefix xs ys
                     in if xs == "abs" || xs == "root" || xs == "plus" || xs == "jmp" || xs == "ifjmp"  then if rest == Nothing then Nothing
